...
- **Intelligent OTP / Verification Code Detection**:
  ...
  - Dates and clock times are not mistaken for codes. A login notification carries the word "OTP" in its own do-not-share warning while containing no code at all, and the last rule in the cascade matches bare digits — which is how the year out of `2026/09/03` used to be reported as the code. A digit run joined by `/ : - .` to a one- or two-digit field is treated as a date or time field and skipped, while `G-483920` and a code split as `1234-5678` still match, and a date early in a message no longer hides a real code later in it.
  - Call-centre numbers are not mistaken for codes either. A bank's logout notice ends with "contact KBZPay Call Center 3211" and its own "employees will never ask for your OTP, PIN or NRC" advice, so the same bare-digit rule used to report the hotline as `3211`. A 4–8 digit run introduced by a phone label — *call center*, *hotline*, *helpline*, *customer service*, *contact*, *dial*, *tel*, *phone*, ဖုန်း, with `at`, `no.` or `number is` in between — is skipped. The label has to run into the number: a full stop or a comma closes the clause, so "We blocked a call. 123456 is your code" keeps its code, and full 09… numbers were never at risk because the cascade stops at eight digits.
...
- **Message Management & Filtering**:
  ...
  - Received messages are kept for **one hour**, then removed from the inbox and from SIM storage. That is the only retention window Settings offers: a code is spent within minutes of arriving, SIM memory holds only a few dozen messages and starts refusing new SMS once it is full, and anything still on the card after that is a code that can still be read off it. The sweep is confirmed against the card rather than assumed — a slot that did not actually go is reported as kept.
